            ...
            <div class="row">
                ...
                <div class="right-pro-detail col-md-6 col-lg-6 mb-0">
                    ...
                    <ul class="attr-pro-detail">
                        ...
                        <?php if (!empty($rowVersion)) { ?>
                            <li class="version-block-pro-detail w-clear">
                                <label class="attr-label-pro-detail d-block">Phiên bản:</label>
                                <div class="attr-content-pro-detail d-block">
                                    <div class="form-row">
                                        <?php foreach ($rowVersion as $k => $v) { ?>
                                            <div class="col__size col-6">
                                                <label for="version-pro-detail-<?= $v['id'] ?>" class="version-pro-detail sale-pro-detail text-decoration-none load_price <?= ($config['LQD']['cartadvanced'] == true && $k == 0) ? 'active' : '' ?>" data-idproduct="<?= $rowDetail['id'] ?>" data-type="<?= $rowDetail['type'] ?>">
                                                    <input type="radio" value="<?= $v['id'] ?>" id="version-pro-detail-<?= $v['id'] ?>" name="version-pro-detail" <?= ($config['LQD']['cartadvanced'] == true && $k == 0) ? 'checked' : '' ?>>
                                                    <?= $v['name' . $lang] ?>
                                                </label>
                                            </div>
                                        <?php } ?>
