...
$version = (!empty($_POST['version'])) ? htmlspecialchars($_POST['version']) : 0;
...
if ($cmd == 'add-cart' && $id > 0) {
    $cart->addToCart($quantity, $id, $color, $size, $version);
    ...
    if (!empty($_SESSION['cart'])) {
        ...
        for ($i = 0; $i < $max; $i++) {
            if ($code == $_SESSION['cart'][$i]['code']) {
                ...
                if ($color) $_SESSION['cart'][$i]['color'] = $color;
                if ($size) $_SESSION['cart'][$i]['size'] = $size;
                if ($version) $_SESSION['cart'][$i]['version'] = $version;
                break;
            }
        }
    }
    ...
    $prosaleinfo = $cart->getProSaleInfo($id, $color, $size, $version);
    if (($color > 0 || $size > 0 || $version > 0) && $prosaleinfo['regular_price'] > 0) {
        $regular_price = $func->formatMoney($prosaleinfo['regular_price'] * $quantity);
        $sale_price = $func->formatMoney($prosaleinfo['sale_price'] * $quantity);
    } else {
            ...
            <div class="top-cart border-right-0">
                <div class="list-procart">
                    ...
                    for ($i = 0; $i < $max; $i++) {
                        ...
                        $version = ($_SESSION['cart'][$i]['version']) ? $_SESSION['cart'][$i]['version'] : 0;
                        ...
                        if (($color > 0 || $size > 0 || $version > 0) && $prosaleinfo['regular_price'] > 0) {
                            $pro_price = $prosaleinfo['regular_price'];
                            $pro_price_new = $prosaleinfo['sale_price'];
                        } else {
                            ...
                            <div class="form-row">
                                ...
                                <div class="info-procart col-6 col-md-5">
                                    ...
                                    <div class="properties-procart">
                                        <?php if ($color) {
                                            ...
                                            <p class="d-block">Màu: <strong><?= $color_detail['name' . $lang] ?></strong></p>
                                        <?php } ?>
                                        
                                        <?php if ($size) {
                                            $size_detail = $d->rawQueryOne("select name$lang from #_size where type = ? and id = ? limit 0,1", array($proinfo['type'], $size)); ?>
                                            <p class="d-block">Size: <strong><?= $size_detail['name' . $lang] ?></strong></p>
                                        <?php } ?>

                                        <?php if ($version) {
                                            $version_detail = $d->rawQueryOne("select name$lang from #_version where type = ? and id = ? limit 0,1", array($proinfo['type'], $version)); ?>
                                            <p class="d-block">Phiên bản: <strong><?= $version_detail['name' . $lang] ?></strong></p>
                                        <?php } ?>
                                ...
                                <div class="quantity-procart col-3 col-md-2">
                                    <div class="price-procart price-procart-rp">
                                        <?php if ($proinfo['sale_price'] || $prosaleinfo['sale_price']) { ?>
                                    ...
                                    <div class="quantity-counter-procart quantity-counter-procart-<?= $code ?>">
                                        ...
                                        <input type="number" class="quantity-procart" min="1" value="<?= $quantity ?>" data-pid="<?= $pid ?>" data-color="<?= $color ?>" data-size="<?= $size ?>" data-version="<?= $version ?>" data-code="<?= $code ?>" />
