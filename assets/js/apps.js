...
NN_FRAMEWORK.Cart = function () {
	...
	$('body').on('click', '.addcart', function () {
		...
		size = size ? size : 0;
		var version = ($this.data('version')) ? $this.data('version') : $parents.find('.version-block-pro-detail').find('.version-pro-detail input:checked').val();
	...
	$('body').on('click', '.counter-procart', function () {
		...
		updateCart(id, code, quantity, color, size, version);
	});
	...
	if (isExist($('.sale-pro-detail'))) {
		$('.sale-pro-detail input').click(function () {
			...
			var id_version = $parents.find('.version-block-pro-detail .version-pro-detail input:checked').val();
	...
	if(CART_ADVANCED){
		function updatePrice(o) {
			...
			var id_version = $parents.find('.version-block-pro-detail').find('.version-pro-detail input:checked').val();
			var id_pro = $this.data('idproduct');
			var type = $this.data('type');

			if (id_color > 0 || id_size > 0 || id_version > 0) {
				$.ajax({
	...
	/* Version */
	if (isExist($('.version-pro-detail'))) {
		$('.version-pro-detail input').click(function () {
			$this = $(this).parents('label.version-pro-detail');
			$parents = $this.parents('.attr-pro-detail');
			$parents.find('.version-block-pro-detail').find('.version-pro-detail').removeClass('active');
			$parents.find('.version-block-pro-detail').find('.version-pro-detail input').prop('checked', false);
			$this.addClass('active');
			$this.find('input').prop('checked', true);
		});
	}
