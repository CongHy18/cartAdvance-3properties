...
class Functions
{
    ...
    public function getSize($id = 0)
    {
       ...
       public function getVersion($id = 0)
       {
           ...
           if ($id) {
               $temps = $this->d->rawQuery("select id_version from #_product_sale where id_parent = ?", array($id));
               $temps = (!empty($temps)) ? $this->joinCols($temps, 'id_version') : array();
               $temps = (!empty($temps)) ? explode(",", $temps) : array();
           }
   
           $row_version = $this->d->rawQuery("select namevi, id from #_version where type = ? order by numb,id desc", array($type));
   
           $str = '<select data-id_parent="' . $id . '" data-type="' . $type . '" id="dataVersion" name="dataVersion[]" class="select multiselect multiselect-cart-advanced" multiple="multiple" >';
           for ($i = 0; $i < count($row_version); $i++) {
               if (!empty($temps)) {
                   if (in_array($row_version[$i]['id'], $temps)) $selected = 'selected="selected"';
                   else $selected = '';
               } else {
                   $selected = '';
               }
               $str .= '<option value="' . $row_version[$i]["id"] . '" ' . $selected . ' /> ' . $row_version[$i]["namevi"] . '</option>';
           }
           $str .= '</select>';
    ...
    public function getProductCat($pid = 0)
    {
        $row = null;
        if ($pid) {
            $row = $this->d->rawQuery("select * from #_product_cat where id_list = ?  and find_in_set('hienthi',status) order by numb,id desc", array($pid));
        }
        return $row;
    }
    ...
    public function getProduct($pid = 0)
    {
        $row = null;
        if ($pid) {
            $row = $this->d->rawQuery("select * from #_product where id_list = ?  and find_in_set('hienthi',status) order by numb,id desc", array($pid));
        }
        return $row;
    }
    ...
    public function viewedProducts($pid = 0)
    {
        if ($pid < 1) return;

        if (!empty($_SESSION['viewed_products'])) {
            $this->productExistsInView($pid);
            $max = count($_SESSION['viewed_products']);
            $_SESSION['viewed_products'][$max]['productid'] = $pid;
        } else {
            $_SESSION['viewed_products'] = array();
            $_SESSION['viewed_products'][0]['productid'] = $pid;
        }
    }
    private function productExistsInView($pid)
    {
        if (!empty($_SESSION['viewed_products'])) {
            foreach ($_SESSION['viewed_products'] as $k => $item) {
                if ($item['productid'] === $pid) {
                    unset($_SESSION['viewed_products'][$k]);
                    $_SESSION['viewed_products'] = array_values($_SESSION['viewed_products']);
                    return true;
                }
            }
        }
