...
if (!empty($_POST['thanhtoan'])) {
    ...
    for ($i = 0; $i < $max; $i++) {
        ...
        $prosaleinfo = $cart->getProSaleInfo($pid,$color,$size);
        ...
        $orderDetailVals = array(
            $proinfo['name' . $lang],
            $text_attr,
            ($color>0||$size>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['sale_price']) : $func->formatMoney($proinfo['sale_price'])) : $func->formatMoney($proinfo['sale_price']),
            ($color>0||$size>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['regular_price']) : $func->formatMoney($proinfo['regular_price'])) : $func->formatMoney($proinfo['regular_price']),
            $q,
            ($color>0||$size>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['sale_price'] * $q): $func->formatMoney($proinfo['sale_price']* $q)):$func->formatMoney($proinfo['sale_price']* $q),
			($color>0||$size>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['regular_price'] * $q): $func->formatMoney($proinfo['regular_price']* $q)):$func->formatMoney($proinfo['regular_price']* $q),
        );

        /* Get order details */
        $order_detail .= str_replace($orderDetailVars, $orderDetailVals, $emailer->markdown('order/details', ['productAttr' => $text_attr, 'salePrice' => ($color>0||$size>0)? ($prosaleinfo['regular_price']>0 ? $prosaleinfo['sale_price'] : $proinfo['sale_price']):$proinfo['sale_price']]));
    }
    ...
    if ($id_insert) {
        for ($i = 0; $i < $max; $i++) {
            ...
            $proinfo = $cart->getProductInfo($pid);
            $prosaleinfo = $cart->getProSaleInfo($pid,$color,$size);
            $regular_price = ($color>0||$size>0)?($prosaleinfo['regular_price']>0 ? $prosaleinfo['regular_price'] : $proinfo['regular_price']) : $proinfo['regular_price'];
            $sale_price = ($color>0||$size>0)? ($prosaleinfo['regular_price']>0? $prosaleinfo['sale_price'] :$proinfo['sale_price']) :$proinfo['sale_price'];
