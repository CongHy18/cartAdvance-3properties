...
class Cart
{
    ...
    public function getProSaleInfo($pid = 0,$id_color=0, $id_size=0)
    {
        $row = null;
        if ($pid) {
    ...
    public function getOrderTotal()
    {
        ...
        if (!empty($_SESSION['cart'])) {
            ...
            for ($i = 0; $i < $max; $i++) {
                ...
                $proinfo = ($color>0 || $size>0) ? ((intval($this->getProSaleInfo($pid,$color,$size)['regular_price'])>0) ? $this->getProSaleInfo($pid,$color,$size) : $this->getProductInfo($pid)) : $this->getProductInfo($pid);
