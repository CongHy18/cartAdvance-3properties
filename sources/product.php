...
if ($id != '') {
    /* Lấy sản phẩm detail */
    $rowDetail = $d->rawQueryOne("select type, id, name$lang, slugvi, slugen, desc$lang, promotions$lang,  content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, options, discount, sale_price, regular_price from #_product where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));

    $func->viewedProducts($id);
    ...
    if (!empty($productColor)) {
        $rowColor = $d->rawQuery("select type_show, photo, name$lang, color, id from #_color where type='" . $type . "' and id in ($productColor) and find_in_set('hienthi',status) order by numb,id desc");
    }
    ...
    /* Lấy Version */
    $productVersion = $d->rawQuery("select id_version from #_product_sale where id_parent = ?", array($rowDetail['id']));
    $productVersion = (!empty($productVersion)) ? $func->joinCols($productVersion, 'id_version') : array();

    if (!empty($productVersion)) {
        $rowVersion = $d->rawQuery("select id, name$lang from #_version where type='" . $type . "' and id in ($productVersion) and find_in_set('hienthi',status) order by numb,id desc");
    }
    ...
    $where = "";
    $where = "id <> ? and id_item = ? and type = ? and find_in_set('hienthi',status)";
    $params = array($id, $rowDetail['id_item'], $type);
    ...
    /* Lấy cấp 1 detail */
    $productList = $d->rawQueryOne("select id, name$lang, slugvi, slugen, type, photo, photo2, options from #_product_list where id = ? and type = ? limit 0,1", array($idl, $type));
    ...
    $url = $func->getCurrentPageURL();
    $paging = $func->pagination($total, $perPage, $curPage, $url);
    $template = isset($_GET['id']) ? "product/product_detail" : "product/product_list";
    ...
    /* Lấy cấp 2 detail */
    $productCat = $d->rawQueryOne("select * from #_product_cat where id = ? and type = ? limit 0,1", array($idc, $type));
    ...
    $url = $func->getCurrentPageURL();
    $paging = $func->pagination($total, $perPage, $curPage, $url);
    $template = isset($_GET['id']) ? "product/product_detail" : "product/product_cat";
    ...
    $url = $func->getCurrentPageURL();
    $paging = $func->pagination($total, $perPage, $curPage, $url);
    $template = isset($_GET['id']) ? "product/product_detail" : "product/product_item";
