...
if (!empty($_POST['thanhtoan'])) {
    ...
    for ($i = 0; $i < $max; $i++) {
        ...
        $prosaleinfo = $cart->getProSaleInfo($pid,$color,$size, $version);
        $text_color = $cart->getProductColor($color);
        $text_size = $cart->getProductSize($size);
        $text_version = $cart->getProductSize($version);
        $text_attr = '';
        if ($text_color != '' && $text_size != '' && $text_version != '') $text_attr = $text_color . " - " . $text_size . " - " . $text_version;
        else if ($text_color != '') $text_attr = $text_color;
        else if ($text_size != '') $text_attr = $text_size;
        else if ($text_version != '') $text_attr = $text_version;
        ...
        $orderDetailVals = array(
            $proinfo['name' . $lang],
            $text_attr,
            ($color>0||$size>0||$version>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['sale_price']) : $func->formatMoney($proinfo['sale_price'])) : $func->formatMoney($proinfo['sale_price']),
            ($color>0||$size>0||$version>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['regular_price']) : $func->formatMoney($proinfo['regular_price'])) : $func->formatMoney($proinfo['regular_price']),
            $q,
            ($color>0||$size>0||$version>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['sale_price'] * $q): $func->formatMoney($proinfo['sale_price']* $q)):$func->formatMoney($proinfo['sale_price']* $q),
			($color>0||$size>0||$version>0)? ($prosaleinfo['regular_price']>0 ? $func->formatMoney($prosaleinfo['regular_price'] * $q): $func->formatMoney($proinfo['regular_price']* $q)):$func->formatMoney($proinfo['regular_price']* $q),
        );

        /* Get order details */
        $order_detail .= str_replace($orderDetailVars, $orderDetailVals, $emailer->markdown('order/details', ['productAttr' => $text_attr, 'salePrice' => ($color>0||$size>0||$version>0)? ($prosaleinfo['regular_price']>0 ? $prosaleinfo['sale_price'] : $proinfo['sale_price']):$proinfo['sale_price']]));
    }
    ...
    if ($id_insert) {
        for ($i = 0; $i < $max; $i++) {
            ...
            $version = $_SESSION['cart'][$i]['version'];
            $q = $_SESSION['cart'][$i]['qty'];
            $proinfo = $cart->getProductInfo($pid);
            $prosaleinfo = $cart->getProSaleInfo($pid,$color,$size,$version);
            $regular_price = ($color>0||$size>0||$version>0)?($prosaleinfo['regular_price']>0 ? $prosaleinfo['regular_price'] : $proinfo['regular_price']) : $proinfo['regular_price'];
            $sale_price = ($color>0||$size>0||$version>0)? ($prosaleinfo['regular_price']>0? $prosaleinfo['sale_price'] :$proinfo['sale_price']) :$proinfo['sale_price'];
            $color = $cart->getProductColor($color);
            $size = $cart->getProductSize($size);
            $version = $cart->getProductVersion($version);
            $code_order = $_SESSION['cart'][$i]['code'];
            ...
            $data_donhangchitiet['version'] = $version;
