...
function printAll(url)
{
    ...
    if(listid == "")
    {
    	notifyDialog("Bạn hãy chọn ít nhất 1 mục để in");
    	return false;
    }

    holdonOpen();
	window.open(url + "&listid=" + listid, '_blank');
	holdonClose();
    /*document.location = url+"&listid="+listid;*/
}
		...
		buttons: {
			success: {
				...
				action: function () {
	            	...
	            	if(action == "print-all") printAll(value);
...
$(document).ready(function () {
    ...
    /* Ajax category */
	if(CART_ADVANCED){
		function updateCartAdvanced(o) {
			var id_parent = o.data('id_parent');
			var type = o.data('type');
			var selectedValuesColor = $("#dataColor option:selected").map(function() { return this.value; }).get().join(", ");
			var selectedValuesSize = $("#dataSize option:selected").map(function() { return this.value; }).get().join(", ");
			var selectedValuesVersion = $("#dataVersion option:selected").map(function() { return this.value; }).get().join(", ");
			var d = {
				id_parent: id_parent,
				type: type,
				ColorList: selectedValuesColor,
				SizeList: selectedValuesSize,
				VersionList: selectedValuesVersion,
			};
		
			$.ajax({
				url: 'api/cartadvanced.php',
				type: 'POST',
				dataType: 'json',
				data: d,
				success: function(data) {
					if(selectedValuesColor!='' || selectedValuesSize!='' || selectedValuesVersion!=''){
						o.parents('.form-box-price').find('.post-list-price').html(data.items);
					}else{
						o.parents('.form-box-price').find('.post-list-price').html('');
					}
				}
			});
		}
		
		$('body').on('change', '.multiselect-cart-advanced', function () {
			updateCartAdvanced($(this));
		});
		
		$('.multiselect-cart-advanced').each(function () {
			updateCartAdvanced($(this));
		});
	}
    ...
    if($("#print-all").length)
    {
		$('body').on('click','#print-all', function(){
			var url = $(this).data("url");
			confirmDialog("print-all","Bạn có chắc muốn in những mục này ?",url);
	    });
    }
	...
	if ($('.form-control-ckeditor').length) {
		CKEDITOR.editorConfig = function (config) {
			...
			/* All Plugins */
			config.extraPlugins =
				'codemirror,texttransform,copyformatting,html5audio,flash,youtube,wordcount,tableresize,widget,lineutils,clipboard,dialog,dialogui,widgetselection,lineheight,video,videodetector,ckeditorfa'+image2;
			...
			/* Config CKFinder - ELFinder */
			if(PHP_VERSION>80000){ config.filebrowserBrowseUrl = 'ckfinder3.6.1/ckfinder.html?token=' + TOKEN; config.filebrowserUploadUrl ='ckfinder3.6.1/core/connector/php/connector.php? command=QuickUpload&type=Files&token=' + TOKEN;}

			if(PHP_VERSION>70000 && PHP_VERSION<80000){ config.filebrowserBrowseUrl = 'ckfinder3.5.2/ckfinder.html?token=' + TOKEN; config.filebrowserUploadUrl ='ckfinder3.5.2/core/connector/php/connector.php?command=QuickUpload&type=Files&token=' + TOKEN;}
