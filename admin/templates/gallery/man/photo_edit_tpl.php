...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <?php if (isset($config[$com][$type][$dfgallery][$val]['cart_photo']) && $config[$com][$type][$dfgallery][$val]['cart_photo'] == true) { ?>
            ...
            $size = $d->rawQuery("select id_size from #_product_sale where id_parent = ?", array($id_parent));
            $size = (!empty($size)) ? $func->joinCols($size, 'id_size') : array();
            $size = (!empty($size)) ? explode(",", $size) : array();

            $version = $d->rawQuery("select id_version from #_product_sale where id_parent = ?", array($id_parent));
            $version = (!empty($version)) ? $func->joinCols($version, 'id_version') : array();
            $version = (!empty($version)) ? explode(",", $version) : array();
            ...
            if (!empty($version)) {
                $cols = ["namevi", "id"];
                $d->where('id', $version, 'IN');
                $d->where('type', $type);
                $result_version = $d->get("version", null, $cols);
            }
                ...
                <div class="card-body">
                    <?php if (isset($result_size) && count($result_size) > 0) {
                        foreach ($result_size as $k => $v) { ?>
                            <div class="custom-control custom-radio d-inline-block mr-3 text-md">
                                <input class="custom-control-input" type="radio" id="id_size<?= $k ?>" name="data[id_size]" <?= ($item['id_size'] == $v['id']) ? 'checked' : '' ?> value="<?= @$v['id'] ?>">
            ...
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
                    <h3 class="card-title">Danh mục phiên bản</h3>
                </div>
                <div class="card-body">
                    <?php if (isset($result_version) && count($result_version) > 0) {
                        foreach ($result_version as $k => $v) { ?>
                            <div class="custom-control custom-radio d-inline-block mr-3 text-md">
                                <input class="custom-control-input" type="radio" id="id_version<?= $k ?>" name="data[id_version]" <?= ($item['id_version'] == $v['id']) ? 'checked' : '' ?> value="<?= @$v['id'] ?>">
                                <label for="id_version<?= $k ?>" class="custom-control-label font-weight-normal"><?= $v['namevi'] ?></label>
                            </div>
                        <?php }
                    } else { ?>
                        <div class="alert alert-warning" role="alert">
