...
switch ($act) {
    ...
    case "man_version":
        viewVersions();
        $template = "product/version/versions";
        break;
    case "add_version":
        $template = "product/version/version_add";
        break;
    case "edit_version":
        editVersion();
        $template = "product/version/version_add";
...
function saveMan()
{
    ...
    $dataVersion = (!empty($_POST['dataVersion'])) ? $_POST['dataVersion'] : null;
    ...
    if ($id && $act != 'save_copy') {
        ...
        if ($d->update('product', $data)) {
            ...
            /* Sale */
            if (!empty($config['product'][$type]['color']) || !empty($config['product'][$type]['size']) || !empty($config['product'][$type]['version'])) {

                if (!empty($dataColor) && !empty($dataSize) && !empty($dataVersion)) {
                    $dataSale1 = array();
                    ...
                    $dataSale3 = array();
                    $dataSale3['id'] = 'id_version';
                    $dataSale3['data'] = $dataVersion;

                    $dataSale4 = array();
                    $dataSale4['id'] = 'regular_price';
                    $dataSale4['data'] = $data_regular_price;

                    $dataSale5 = array();
                    $dataSale5['id'] = 'sale_price';
                    $dataSale5['data'] = $data_sale_price;

                    $dataSale6 = array();
                    $dataSale6['id'] = 'discount';
                    $dataSale6['data'] = $data_discount;
                } else if (!empty($dataColor) && !empty($dataSize) && empty($dataVersion)) {
                    ...
                    $dataSale6['id'] = 'discount';
                    $dataSale6['data'] = $data_discount;
                } else if (!empty($dataColor) && !empty($dataVersion)  && empty($dataSize)) {
                    $dataSale1 = array();
                    $dataSale1['id'] = 'id_color';
                    $dataSale1['data'] = $dataColor;
                    ...
                    $dataSale6['id'] = 'discount';
                    $dataSale6['data'] = $data_discount;
                } else if (!empty($dataSize) && !empty($dataVersion)  && empty($dataColor)) {
                    $dataSale2 = array();
                    $dataSale2['id'] = 'id_size';
                    $dataSale2['data'] = $dataSize;
                    ...
                    $dataSale6['id'] = 'discount';
                    $dataSale6['data'] = $data_discount;
                } else if (!empty($dataSize) && empty($dataColor)  && empty($dataVersion)) {
                    $dataSale2 = array();
                    $dataSale2['id'] = 'id_size';
                    $dataSale2['data'] = $dataSize;
                    ...
                    $dataSale6['id'] = 'discount';
                    $dataSale6['data'] = $data_discount;
                } else if (!empty($dataVersion) && empty($dataColor)  && empty($dataSize)) {
                    $dataSale3 = array();
                    $dataSale3['id'] = 'id_version';
                    $dataSale3['data'] = $dataVersion;
                ...
                if (!empty($dataSale1['data']) || !empty($dataSale2['data']) || !empty($dataSale3['data'])) {
                                ...
                                foreach ($dataSale2['data'] as $v_sale2) {
                                    ...
                                    if (!empty($dataSale3['data'])) {
                                        foreach ($dataSale3['data'] as $v_sale3) {
                                            ...
                                            if (!empty($dataSale6['data'])) {
                                                $dataSale[$dataSale6['id']] = array_shift($dataSale6['data']);
                                            }
                
                                            $d->insert('product_sale', $dataSale);
                                        }
                                    } else {
                                        // Trường hợp chỉ có $dataSale1 và $dataSale2
                                        if (!empty($dataSale4['data'])) {
                                            $dataSale[$dataSale4['id']] = array_shift($dataSale4['data']);
                                        }
                    ...
                    // Trường hợp chỉ có $dataSale2 và $dataSale3
                    if (empty($dataSale1['data']) && !empty($dataSale2['data']) && !empty($dataSale3['data'])) {
                        foreach ($dataSale2['data'] as $v_sale2) {
                            $dataSale = array();
                            $dataSale['id_parent'] = $id;
                    ...
                    if (!empty($dataSale1['data'])) {
                        foreach ($dataSale1['data'] as $v_sale1) {
                            ...
                            if (!empty($dataSale2['data'])) {
                                ...
                                // Nếu $dataSale2['data'] không tồn tại, kiểm tra $dataSale3['data']
                                if (!empty($dataSale3['data'])) {
                                    foreach ($dataSale3['data'] as $v_sale3) {
                                        $dataSale[$dataSale3['id']] = $v_sale3;
...
function viewVersions()
{
    ...
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select * from #_version where type = ? $where order by numb,id desc $limit";
    $items = $d->rawQuery($sql, array($type));
    $sqlNum = "select count(*) as 'num' from #_version where type = ? $where order by numb,id desc";
    $count = $d->rawQueryOne($sqlNum, array($type));
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "index.php?com=product&act=man_version&type=" . $type;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editVersion()
{
    ...
    if (empty($id)) {
        $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_version where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
...
function saveVersion()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("index.php?com=product&act=edit_version&type=" . $type . "&p=" . $curPage . "&id=" . $id);
        } else {
            $func->redirect("index.php?com=product&act=add_version&type=" . $type . "&p=" . $curPage);
        }
    }
    ...
    if ($id) {
        ...
        $d->where('id', $id);
        $d->where('type', $type);
        if ($d->update('version', $data)) {
            $func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage, false);
        }
    } else {
        $data['date_created'] = time();

        if ($d->insert('version', $data)) {
            $func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
...
function deleteVersion()
{
    ...
    if ($id) {
        $row = $d->rawQueryOne("select id from #_version where id = ? and type = ? limit 0,1", array($id, $type));

        if (!empty($row)) {
            $d->rawQuery("delete from #_version where id = ? and type = ?", array($id, $type));

            /* Xóa version in Sale */
            $d->rawQuery("delete from #_product_sale where id_version = ?", array($id));

            $func->transfer("Xóa dữ liệu thành công", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_version&type=" . $type . "&p=" . $curPage, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
    ...
    if ($id) {
        ...
        if ($d->update('product_cat', $data)) {
            ...
            if ($func->hasFile("file2")) {
                ...
                if ($photo = $func->uploadImage("file2", $config['product'][$type]['img_type_cat'], UPLOAD_PRODUCT, $file_name)) {
                    $row = $d->rawQueryOne("select id, photo2 from #_product_cat where id = ? and type = ? limit 0,1", array($id, $type));
            ...
            if ($func->hasFile("file3")) {
                ...
                if ($photo = $func->uploadImage("file3", $config['product'][$type]['img_type_cat'], UPLOAD_PRODUCT, $file_name)) {
                    $row = $d->rawQueryOne("select id, photo3 from #_product_cat where id = ? and type = ? limit 0,1", array($id, $type));

                    if (!empty($row)) {
                        $func->deleteFile(UPLOAD_PRODUCT . $row['photo3']);
                    }

                    $photoUpdate['photo3'] = $photo;
                    $d->where('id', $id);
                    $d->update('product_cat', $photoUpdate);
                    unset($photoUpdate);
                }
...
function deleteCat()
{
    ...
    if ($id) {
        /* Lấy dữ liệu */
        $row = $d->rawQueryOne("select id, photo, photo2, photo3 from #_product_cat where id = ? and type = ? limit 0,1", array($id, $type));
